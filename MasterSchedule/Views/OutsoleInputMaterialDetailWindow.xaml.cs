        ...
        private void bwLoadDetail_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            foreach (var indexNo in indexNoList)
            {
                ...
                foreach (var round in roundList)
                {
                    ...
                    {
                        var sizeRun = sizeRunList[i];
                        var outsoleMaterialDetailPerIndexPerRoundPerSizeList = outsoleMaterialDetailPerIndexPerRoundList.FirstOrDefault(f => f.SizeNo == sizeRun.SizeNo);
            ...
            {
                string revisePeople = accountList.FirstOrDefault(f => f.UserName == outsoleMaterialDetailReloadFirstDefault.CreatedBy).FullName;
